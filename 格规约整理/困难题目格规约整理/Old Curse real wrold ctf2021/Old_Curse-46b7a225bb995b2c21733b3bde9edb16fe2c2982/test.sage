...
def lattice_attack(pol, e, X, Y, Z, mm, tt):
    ...
    for kk in range(mm + 1):
        ...
        for i2 in range(kk + 1, i1 + tt + 1):
            i3 = mm - i1
            poly = y ^ (i2 - kk) * z ^ i3  * e ^ (mm - kk)
            polys.append(poly)
    print(polys)
    ic(polys[0])
    # ic(polys[0](x*X))
    ic(polys[0]( x, y * Y, z * Z))
    input()
    polys.sort()
    ...
    for ii in range(dims1):
        for jj in range(dims2):
            if monomials[jj] in polys[ii].monomials():
                ic(ii,jj)
                ic(monomials[jj], polys[ii], polys[ii](x * X, y * Y, z * Z).monomial_coefficient(monomials[jj]))
                ic(polys[ii](x * X, y * Y, z * Z))
                M[ii, jj] = polys[ii](x * X, y * Y, z * Z).monomial_coefficient(monomials[jj])
                input()
